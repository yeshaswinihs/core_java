...
public class JWTWebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.headers().frameOptions().sameOrigin() // H2 Console Needs
															// this setting
	...
	public void configure(WebSecurity webSecurity) throws Exception {
		webSecurity.ignoring().antMatchers(HttpMethod.POST, authenticationPath).antMatchers(HttpMethod.OPTIONS, "/**")
				.and().ignoring().antMatchers(HttpMethod.GET, "/" // Other Stuff
																	// You want
																	// to Ignore
				).and().ignoring().antMatchers("/h2-console/**/**")
				.and().ignoring().antMatchers(HttpMethod.POST, "/submitUser");
//				.and().ignoring().antMatchers(HttpMethod.OPTIONS, "/submitUser");// Should
																	// not be in
																	// Production!
	}
}
